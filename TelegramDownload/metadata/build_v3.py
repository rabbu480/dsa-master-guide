...
.page {
    background: white;
    max-width: 1100px; /* A4-ish proportions for screen */
...
/* WIDE BOXES FOR TABLES AND MERMAID CHARTS */
.span-all {
...
def deduplicate_sections(soup):
    ...
    for box in soup.find_all('div', class_='section-box'):
        ...
        if header:
            ...
            if 'INTERVIEW FLOW' in title: title = 'INTERVIEW FLOW'
...
def should_span_all(box):
    ...
    if header:
        ...
        if 'INTERVIEW THINKING FLOW' in title: return True
...
def build_v3():
    ...
    print(f"Building v3 with PROPER MASONRY for {len(finals)} topics...")
    
    for fname in sorted(finals):
        src_path = os.path.join(src_dir, fname)
        dst_path = os.path.join(v3_dir, fname)
        ...
        # REMOVE INLINE STYLES FOR CLEANER CSS
        for tag in soup.find_all(True):
            if tag.get('style'):
                # keep colors for code highlighting or specific diff tags
                if not any(c in tag['style'] for c in ['color:', 'background-color:', 'background:']):
                    del tag['style']
        ...
        for style in soup.find_all('style'):
            if not style.get('id') == 'faang-v3':
                style.decompose()
            
        head = soup.find('head')
        if head:
            head.append(BeautifulSoup(V3_CSS, 'html.parser'))
            
        body = soup.find('body')
        if body:
            # We already have script tag injected? Let's check if it exists
        ...
        print(f"  [{size_kb:.0f}KB] {fname} - Added Platinum CSS.")
