...
print("Raw page blocks in body:", len(raw_pages))

# Format exactly 5 pages!
target_count = 5
chunk_size = (len(raw_pages) + target_count - 1) // target_count
grouped = []
for i in range(0, len(raw_pages), chunk_size):
    grouped.append(raw_pages[i:i+chunk_size])
...
for idx, grp in enumerate(grouped):
    ...
    for inner_idx, p in enumerate(grp):
        p_str = p if p.startswith('<div class="ph">') else f'<div class="ph">{p}'
        if inner_idx > 0:
    ...
    # Clean unclosed page divs inside combined block
...
print(f"=== REBUILT TOPIC 03 INTO EXACTLY {len(formatted_pages)} TOP-LEVEL PAGE DIVS ===")
