...
v4_dir = r"C:\Users\rabba\Downloads\TelegramDownload\metadata\v4"
...
.box-aha { background-color: #f0fdf4; border: 2px solid #22c55e; border-radius: 8px; padding: 20px 15px 15px; position: relative; box-shadow: 0 2px 5px rgba(34,197,94,0.1); }
...
.box-tip { background-color: #fffbeb; border: 2px solid #f59e0b; border-radius: 8px; padding: 20px 15px 15px; position: relative; box-shadow: 0 2px 5px rgba(245,158,11,0.1); }
...
.box-mistake { background-color: #fef2f2; border: 2px solid #ef4444; border-radius: 8px; padding: 20px 15px 15px; position: relative; box-shadow: 0 2px 5px rgba(239,68,68,0.1); }
...
@media print {
    ...
    .section-box, .box-aha, .box-tip, .box-mistake, .toc-box, .toc-page-col, .content-masonry > div { 
...
def transform_interview_flow(soup, box):
    ...
    # Check if it's the specific Interview Flow format: numbers, titles, arrows
    text = content_div.get_text()
    if 'Clarify' in text and 'Constraints' in text and 'Brute Force' in text:
        # Reconstruct as a beautiful flex flow
        steps = [
        ...
        box['class'] = box.get('class', []) + ['span-all'] # Make this horizontal flow span full width
...
def should_span_all(box):
    ...
    if header:
        ...
        # Let INTERVIEW FLOW span-all but using the horizontal layout we injected
...
def build_v4():
    ...
    print(f"Building v4 with PLATINUM CSS + FIXED VISIBILITY + SMART ALIGNMENT...")
    
    for fname in sorted(finals):
        src_path = os.path.join(src_dir, fname)
        dst_path = os.path.join(v4_dir, fname)
        ...
        for tag in soup.find_all(True):
            if tag.name not in ['pre', 'code'] and not tag.find_parent('pre') and not tag.find_parent('code'):
                style = tag.get('style', '')
                if style:
                    # Remove "color: white", "color: #fff", "color: #ffffff"
        ...
        # Strip all inline styles EXCEPT colors on code blocks to keep DOM incredibly clean
        for tag in soup.find_all(True):
            if tag.get('style'):
        ...
        for style in soup.find_all('style'):
            if not style.get('id') == 'faang-v4':
                style.decompose()
            
        head = soup.find('head')
        if head:
            head.append(BeautifulSoup(V4_CSS, 'html.parser'))
        ...
        print(f"  [{size_kb:.0f}KB] {fname} - Added Platinum CSS v4.")
