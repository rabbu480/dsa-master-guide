...
v8_dir = r"C:\Users\rabba\Downloads\TelegramDownload\metadata\v8"
...
body {{
    ...
    margin: 0;
    padding: 10px 20px;
    font-size: 11.5px; /* Slightly smaller for compactness */
    line-height: 1.45;
...
.mermaid svg {{ max-width: 100% !important; height: auto !important; max-height: 220px !important; display: block; margin: 0 auto; }}
...
@media print {{
    ...
    .master-container {{ margin: 0 !important; max-width: none !important; }}
    .content-grid {{ display: flex; flex-direction: column; }} /* On print, allow standard flow if grid fails, but grid is fine usually */
...
def build_v8():
    ...
    print(f"Building v8 FINAL (Ultra Polish)...")
    
    for fname in sorted(finals):
        src_path = os.path.join(src_dir, fname)
        dst_path = os.path.join(v8_dir, fname)
        ...
        head.append(BeautifulSoup(get_v8_css(theme), 'html.parser'))
        ...
        print(f"  [{size_kb:.0f}KB] {fname} - Finalized V8 (No TOC, Stacked cols, API updates).")
