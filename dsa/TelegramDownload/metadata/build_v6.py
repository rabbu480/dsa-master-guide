...
v6_dir = r"C:\Users\rabba\Downloads\TelegramDownload\metadata\v6"
...
# Completely removed the corrupted pre.innerHTML JS replace logic!
JS_SYNTAX = """
<script type="module">
...
def should_span_all(box):
    ...
    if header:
        ...
        # Crucial fix for Box 4 array representation: do NOT span all
...
def deduplicate_and_restructure(soup):
    ...
    # 1. First pass to find specific boxes to embed (like 8 and 9 into 7)
    term_box = None
    points_box = None
    mistakes_box = None
    ...
    for box in soup.find_all('div', class_='section-box'):
        ...
        if 'IMPORTANT TERMINOLOGY' in title:
            term_box = box
        elif 'KEY POINTS TO REMEMBER' in title:
            points_box = box
        elif 'COMMON MISTAKES' in title:
            mistakes_box = box
            
    # Combine 8 & 9 into 7 if all exist
...
def build_v6():
    ...
    print(f"Building v6 (Perfect Recreation)...")
    
    for fname in sorted(finals):
        src_path = os.path.join(src_dir, fname)
        dst_path = os.path.join(v6_dir, fname)
        ...
        # Prevent mermaid parse errors
        clean_mermaid(soup)
        
        # Ensure box 3 (Complete binary tree) uses flex-row correctly
        ...
        # CREATE A SEPARATE PAGE FOR TOC
        if toc_by_page:
            toc_page = soup.new_tag('div', attrs={'class': 'page'})
            
            toc_header_top = soup.new_tag('div', attrs={'class': 'header-top'})
            toc_title_box = soup.new_tag('div')
            h1 = soup.new_tag('h1', style='color: var(--primary); font-size: 2.5rem; margin: 0; font-weight: 800; letter-spacing: 2px;')
            h1.string = "WHAT'S INSIDE THIS CHEAT SHEET"
            toc_title_box.append(h1)
            toc_header_top.append(toc_title_box)
            
            page_num_div = soup.new_tag('div', attrs={'class': 'page-number'})
            page_num_div['style'] = 'background-color: var(--primary); color: white; font-weight: bold; padding: 8px 16px; border-radius: 20px; font-size: 1.2rem;'
            page_num_div.string = "PAGE 2"
            toc_header_top.append(page_num_div)
            
            toc_page.append(toc_header_top)
            ...
            for page_num, sections in toc_by_page:
                if p_idx == 2:
                    p_idx = 3 
                actual_page_mapping[page_num] = p_idx
                p_idx += 1
            ...
            for page_num, sections in toc_by_page:
                ...
                target_page = actual_page_mapping[page_num]
                page_title.string = f"PAGE {target_page}"
            ...
            toc_page.append(toc_div)
            
            first_page = soup.find('div', class_='page')
            if first_page:
                # Update page 1's header
                p1_header = first_page.find('div', class_='page-number')
                if p1_header: p1_header.string = "PAGE 1"
                
                first_page.insert_after(toc_page)
                
            # Update subsequent pages
            pages = soup.find_all('div', class_='page')
            for i, p in enumerate(pages):
                if i >= 2:
                    p_hdr = p.find('div', class_='page-number')
                    if p_hdr: p_hdr.string = f"PAGE {i + 1}"

        for style in soup.find_all('style'):
            if not style.get('id') == 'faang-v6':
                style.decompose()
            
        head = soup.find('head')
        if head:
            head.append(BeautifulSoup(V6_CSS, 'html.parser'))
        ...
        print(f"  [{size_kb:.0f}KB] {fname} - Added strict v6 layouts.")
