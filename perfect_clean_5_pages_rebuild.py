...
# Strip top <div class="ph">...</div> from raw pages 1, 3, 5, 7, 9!
grouped = [
...
for idx, grp in enumerate(grouped):
    ...
    for inner_idx, p in enumerate(grp):
        # strip outer closing </div> from p
        p_clean = re.sub(r'</div>\s*$', '', p.strip())
        if inner_idx > 0:
            # strip ph block completely
            p_clean = re.sub(r'<div class=["\']ph["\']>[\s\S]*?</div>\s*</div>', '', p_clean)
...
print("=== PERFECT CLEAN 5 PAGE REBUILD WITHOUT LEAF PH CONFLICTS ===")
