...
# Strip the ENTIRE <div class="ph">...</div> block from even-indexed pages (1, 3, 5, 7, 9)
grouped = [
...
for idx, grp in enumerate(grouped):
    ...
    for inner_idx, p in enumerate(grp):
        ...
        if inner_idx > 0:
            # strip <div class="ph">...</div> completely!
            # ph block is between <div class="ph"> and the matching closing </div>
            ph_match = re.search(r'<div class=["\']ph["\']>[\s\S]*?</div>\s*</div>', p_clean)
            if ph_match:
                p_clean = p_clean[ph_match.end():]
            else:
...
print("=== PERFECT CLEAN 5 PAGE REBUILD WITH EXACT PH REMOVAL ===")
