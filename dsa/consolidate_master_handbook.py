...
def consolidate_topic(topic_num_str):
    # Find matching file in v4 or bookfinal
    pattern = f"Topic{topic_num_str}_*.html"
    v4_matches = glob.glob(os.path.join(v4_dir, pattern))
    if not v4_matches:
        v4_matches = glob.glob(os.path.join(book_dir, pattern))
    if not v4_matches:
        print(f"Skipping {topic_num_str}: file not found.")
        return
    ...
    # Topic 01 & 02 are already reference standard! Just ensure CSS is clean.
    if topic_num_str in ["01", "02"]:
        print(f"[{filename}] - Standard Reference Topic, updating CSS print rules.")
        with open(src_file if os.path.exists(src_file) else dst_file, "r", encoding="utf-8") as f:
            html = f.read()
        ...
        print(f"  -> Generated {filename} PDF: {len(doc)} pages.")
        doc.close()
        return
    ...
    topic_title = title_m.group(1) if title_m else f"TOPIC {topic_num_str}"
    ...
    # Group 10 original pages into 5-6 dense pages
    total_orig = len(pages_clean)
    merged_pages = []
    
    if total_orig == 10:
        # Standard 10 -> 5 page consolidation mapping
        groups = [
            (0, 1, "PAGE 1 OF 5", "FOUNDATION & TEMPLATES"),
            (2, 3, "PAGE 2 OF 5", "CORE PATTERNS — PART 1 & 2"),
            (4, 5, "PAGE 3 OF 5", "CORE PATTERNS — PART 3 & 4"),
            (6, 6, "PAGE 4 OF 5", "DECISION TREE & TRIGGERS"),
            (7, 8, "PAGE 5 OF 5", "PROBLEM LADDER & SUMMARY"),
            (9, 9, "PAGE 5 OF 5", "DRY RUN & CHEAT SHEET") # adjust below
        ]
        
        # Let's do 5 or 6 pages:
        # P1: 0+1
        # P2: 2+3
        # P3: 4+5
        # P4: 6 (Decision tree)
        # P5: 7+8 (Problem ladder 1&2 + summary)
        # P6: 9 (Cheat sheet & proofs)
        ...
        for i in range(0, total_orig, 2):
            ...
            p_block = f"""<div class="page"><div class="ph"><div><h1>{topic_title}</h1><div class="sub">FAANG Master Guide</div></div><div style="text-align:right"><div class="pn">PAGE {p_idx}</div></div></div>{p_content}</div>"""
    ...
    doc = fitz.open(pdf_file)
    print(f"[{filename}] -> Consolidated from {total_orig} to {len(merged_pages)} HTML divs | Generated PDF Pages: {len(doc)}")
    doc.close()

# Process all topics 01 to 19
topic_ids = [f"{i:02d}" for i in range(1, 20)]
print("Starting Handbook Consolidation...")
for tid in topic_ids:
    consolidate_topic(tid)

print("\n==========================================")
print("ALL TOPICS CONSOLIDATION COMPLETE!")
print("==========================================")
